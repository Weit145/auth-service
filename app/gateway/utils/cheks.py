...
async def check_reg(
    ...
    user_email = await SQLAlchemyAuthRepository().get_user_by_email(
        request.email, context
    )
    if user_email is not None:
        await context.abort(
            grpc.StatusCode.INVALID_ARGUMENT, "Email alreaddy registreate"
        )

    user_login = await SQLAlchemyAuthRepository().get_user_by_login(
        request.login, context
    )
    if user_login is not None:
        await context.abort(
            grpc.StatusCode.INVALID_ARGUMENT, "Username alreaddy registreate"
        )
